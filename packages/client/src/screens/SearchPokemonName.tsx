...
import * as Pokemon from '../features/pokemon/PokemonByNameSlice'
...
const SearchPokemon = () => {
    ...
    const searchLastWorld = useSelector(Pokemon.searchSelector)
    ...
    const searchFunction = (showMore?: boolean) => {
        // dispatch(Pokemon.setLimit(counterResult))
    ...
    useEffect(() => {
        reloadData()
    }, [useSelector(Pokemon.pokemonSelector), useSelector(Pokemon.limitSelector), useSelector(Pokemon.searchSelector)])

    const reloadData = () => {
        //  alert(limitStore)
        setCounterLimit(limitStore)
        setSearch(searchLastWorld)
        let element: any = []
        for (let i = 0; i < dataStore.length; i++) {
            element.push(dataStore[i].node)
            element[i] = { ...element[i], image: "https://pokeres.bastionbot.org/images/pokemon/" + parseInt(element[i].id, 10) + ".png" }
